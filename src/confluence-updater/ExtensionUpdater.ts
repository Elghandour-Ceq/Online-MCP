...
export abstract class ExtensionUpdater {
    ...
    async getNewVersionAndInstall(): Promise<void> {
        ...
        try {
            ...
            newVersion = await this.showProgress(`Checking for updates for ${this.extensionManifest.displayName}`, () =>
                this.getNewVersion());
        } catch (error) {
            console.error('Failed to check for new version:', error);
            throw new Error(`Unable to check for new version: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }

        if (newVersion) {
            if (this.options?.reInstall || await this.consentToInstall(newVersion)) {
                // 2. download
                console.log("download url:",newVersion.downloadUrl);
    ...
    protected async download(downloadUri: Uri): Promise<Uri> {
        ...
        return new Promise<Uri>((resolve, reject) => {
            const followRedirect = (url: string) => {
                ...
                const req = protocol.request(options, (resp) => {
                    ...
                    if (resp.statusCode && resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location) {
                        ...
                        followRedirect(resp.headers.location);
                        return;
                    }

                    if (resp.statusCode && resp.statusCode >= 400) {
                        console.error(`Download failed with status code: ${resp.statusCode}`);
            ...
            followRedirect(downloadUri.toString());
        });
    }
    ...
    private async getNewVersion(): Promise<ExtensionVersion | undefined> {
        try {
            ...
            throw new Error(`Unable to retrieve latest version: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
