...
The Zaki extension exposes an API that can be used by other extensions. To use this API in your extension:
...
3. Get access to the API with the following code:
    ...
    if (!clineExtension?.isActive) {
    	throw new Error("Zaki extension is not activated")
    }
    ...
    if (cline) {
    	...
    	await cline.startNewTask("Hello, Zaki! Let's make a new project...")
    	...
    	console.error("Zaki API is not available")
    }
    ```
